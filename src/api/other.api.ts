...
import { ArticlesApiResponse, GenerateApiResponse, IndustryListApiResponse, TIndustry } from "@/types/response.types"
...
export async function getArticles(limit = 3, offset = 0): Promise<ArticlesApiResponse> {
    try {
        const res = await axios.get(`${apiConfig.basepath}?limit=${limit}&offset=${offset}`)
        if (res.status === 200 && res.data.status) {
            return {
